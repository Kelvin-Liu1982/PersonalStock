...
步骤:
    1) build_financials.py  重新生成 数据源/中远海控_2021-2025年报及2026Q1财报核心数据.xlsx
                           （联网抓取东方财富业绩报表；失败则回退到本地 财报原始数据_缓存.json）
    2) build_buyback.py     重新生成 数据源/中远海控_2026回购明细.xlsx（内置已核对数据）
    3) build_tracker.py     读取 数据源/ 下全部 xlsx，生成时序数据 + 趋势图(base64)，写入 _data_js.txt
    4) render_html.py       读取 _data_js.txt，合成单文件 HTML（中远海控追踪看板(方向C).html）
