...
=====================================================================
数据主源: 中远海控分析_行业供需与运价数据库(2025-2026).xlsx
...
XLSX = os.path.join(SRC, "中远海控分析_行业供需与运价数据库(2025-2026).xlsx")
...
# ---------- 核心运营指标数据库（季度货运量等，来自另一 xlsx） ----------
OPX = os.path.join(SRC, "中远海控核心运营指标数据库(2021-2026).xlsx")
...
FIN_XLSX = os.path.join(SRC, "中远海控_2021-2025年报及2026Q1财报核心数据.xlsx")
...
fin_div = sheet_to_dicts("分红数据(2021-2025)")
...
fin_cost = sheet_to_dicts("成本结构(集装箱航运业务,2021-2025)")
...
fin_port = sheet_to_dicts("港口码头业务(2021-2025)")
...
# 航线结构占比（集装箱航运业务，分航线箱量与收入，2021–2025 全年，来源：核心运营指标数据库(2021-2026).xlsx）
ROUTE_XLSX = os.path.join(SRC, "中远海控核心运营指标数据库(2021-2026).xlsx")
...
VALUES = {
    ...
    # 股东回报（分红来源：财报核心数据.xlsx；回购来源：中远海控_2026回购明细.xlsx）
    ...
    # 航线结构占比（来源：核心运营指标数据库(2021-2026).xlsx 分航线箱量与收入明细）
    "route_year": "2025",
    "route_mix25": route_mix25,
    # 季度航运货运量（来源：核心运营指标数据库(2021-2026).xlsx 季度货运量明细(2021-)）
    "qvol25": f"{qvol[-1]:.2f}",
...
BUYBACK = {
  ...
  "a_round5": {"as_of":"2026-07-31","shares":29116504,"amt":4.22,"cap":15.40,
...
DATA_JS = "__SERIES__=" + json.dumps(SERIES, ensure_ascii=False) + ";\n" \
        ...
        + "__META__=" + json.dumps({
            ...
            "qvol_range": f"季度货运量明细(2021-)，{qvol_dates[0]} ~ {qvol_dates[-1]}（{len(qvol_dates)} 季）· 来源：核心运营指标数据库(2021-2026).xlsx",
            "gen_time": "2026-08-06",
        }, ensure_ascii=False) + ";"
